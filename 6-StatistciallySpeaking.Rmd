...
author:
  - name: "Group Name: Statistically Speaking"
  - name: "Shreya Mohan, Kalyani Mantirraju, Crystal Arevalo,"
  - name: "Karen Alvarez, Mason Lam, Eric Yang"
date: "2025-04-27"
format:
  html:
    toc: true
...
levene_result <- leveneTest(arr_delay ~ age_group, data = flights_planes)
print(levene_result)

# Since homogeneity is violated, run Kruskal-Wallis test (non-parametric)
kruskal_result <- kruskal.test(arr_delay ~ age_group, data = flights_planes)
...
# Convert manufacturer to factor to avoid warnings in tests
...
# If Kruskal-Wallis is significant, do Dunn's test for pairwise comparisons
dunn_result <- dunn.test(flights_manufacturer$arr_delay, flights_manufacturer$manufacturer, method = "bonferroni")
  
#Instead of printing all pairwise comparisons (which is long), filter and print only significant comparisons:
