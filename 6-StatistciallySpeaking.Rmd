...
anova_age <- aov(arr_delay ~ age_group, data = flights_planes)
summary(anova_age)
TukeyHSD(anova_age)
...
```{r}
delay_by_manufacturer <- flights_planes %>%
  group_by(manufacturer) %>%
  summarise(
    mean_delay = mean(arr_delay, na.rm = TRUE),
    count = n()
  ) %>%
  filter(count > 50) %>%  # Filter to manufacturers with enough data for reliable stats
  arrange(mean_delay)

print(delay_by_manufacturer)

```

```{r}
# Run ANOVA on filtered data
anova_manufacturer <- aov(arr_delay ~ manufacturer, data = flights_planes %>% filter(manufacturer %in% delay_by_manufacturer$manufacturer))

# Summary of ANOVA
summary(anova_manufacturer)

# Run TukeyHSD on the ANOVA model
tukey_manufacturer <- TukeyHSD(anova_manufacturer)

# Convert TukeyHSD result for 'manufacturer' to a data.frame for easier filtering
res_df <- as.data.frame(tukey_manufacturer$manufacturer)
res_df %>%
  # Add comparison names as a column (rownames to column)
  tibble::rownames_to_column(var = "comparison") %>%
  # Select relevant columns
  select(comparison, diff, lwr) %>%
  # Show top 5 rows
  head(5) %>%
  print()
# Show only significant comparisons (adjusted p-value < 0.05)
signif_res <- res_df[res_df$`p adj` < 0.05, ]
print(signif_res)

# Sort by absolute difference
res_df_sorted <- res_df[order(-abs(res_df$diff)), ]
